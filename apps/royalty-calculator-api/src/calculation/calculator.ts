...
/**
 * Calculation engine - shared logic (mirrors @merch/calculation-engine)
...
export function calculate(input: CalculatorInput): CalculatorResult {
  ...
  const revenue = round2(salePricePerUnit * units);
  ...
  let royaltyAmount = 0;
  const royaltyPercent = input.royaltyPercent ?? 0;
  ...
  const totalCost = round2(cost * units);
